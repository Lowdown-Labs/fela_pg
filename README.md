[![CI](https://github.com/Lowdown-Labs/fela_pg/actions/workflows/ci.yml/badge.svg)](https://github.com/Lowdown-Labs/fela_pg/actions/workflows/ci.yml)
...
```bash
docker run -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d ghcr.io/lowdown-labs/fela_pg:pg18
...
- **e2e**: packages onto a throwaway stock PG18 cluster, `CREATE EXTENSION pg_fela`, then runs the
  golden honesty gate, the full AutoML showcase, and a latency check
  (`test/run_pgrx_test.sh`).

**Honesty gate.** Two independent checks, both self contained:
